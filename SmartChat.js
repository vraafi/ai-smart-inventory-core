...
**User Request:** "${wipeCmd}"`;
    ...
    if (lowerText.startsWith("/onboarding")) {
       ...
       const prompt = `**Task:** Ekstrak data registrasi barang baru dari teks berikut:\n\n"${fullContext}"\n\n**CRITICAL CONSTRAINTS - VIOLATING THESE WILL CAUSE A SYSTEM CRASH:**\n- DO NOT print "User request:", "Interpretation:", or "Action:".\n- DO NOT use bullet points or asterisks (*).\n- YOU MUST START YOUR ENTIRE RESPONSE WITH '[' OR '{' and END WITH ']' OR '}'.\n\n**Output Format Example:**\n[{"new_item_name":"nama_barang_lengkap", "quantity":jumlah_stok, "new_price":harga_jual, "buy_price":harga_beli, "branch":"nama_cabang", "new_category":"kategori_barang"}]\n\nJika data SANGAT membingungkan sehingga Anda TIDAK BISA menebak nama barangnya sama sekali, keluarkan JSON: { "cmd": "ASK_USER", "question": "Tuliskan pertanyaan spesifik Anda di sini, JANGAN gunakan titik-titik (...)" }\n\n**Rules:**\n- Ekstrak sebanyak mungkin data yang ada (seperti branch, harga, stok). \n- Jika ada data yang tidak disebutkan, abaikan (jangan dimasukkan ke JSON) atau setel ke 0.\n- SELALU utamakan mengekstrak menjadi array of JSON daripada meminta klarifikasi, asalkan ada nama barang.`;
