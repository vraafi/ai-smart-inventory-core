...
function pollEmails() {
  ...
  try {
    ...
    for (const thread of threads) {
      if (globalProcessedCount >= 1) break; // Hard limit to 1 per trigger to avoid Groq 12k TPM rate limit
      
      const messages = thread.getMessages();
      let threadProcessedCount = 0;
      
      // Process backwards (newest message first)
...
// Combine Subject and Body for AI Context
        ...
        _processWithAI(senderEmail, fullContext, senderName, "Email", bulkDataArray);
...
function _sendWhatsApp(phone, text) {
  ...
  function _sendEmailNotification(recipient, subject, body) {
    try {
      // Strip emojis to prevent encoding issues (Broken chars in Gmail snippet)
      const cleanSubject = subject.replace(/[\u{1F600}-\u{1F6FF}\u{1F300}-\u{1F5FF}\u{1F900}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu, '').trim();
      const cleanBody = body.replace(/[\u{1F600}-\u{1F6FF}\u{1F300}-\u{1F5FF}\u{1F900}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu, '');
      
      // Send both plain text and HTML versions
      GmailApp.sendEmail(recipient, cleanSubject, cleanBody.replace(/<br>/g, "\n").replace(/<[^>]+>/g, ""), { htmlBody: cleanBody });
    } catch (e) {
